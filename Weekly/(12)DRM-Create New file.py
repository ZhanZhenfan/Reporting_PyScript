...
def safe_excel_app():
    ...
    try:
        ...
        warn("部分 Excel 优化属性设置失败。")
    return excel

def find_latest_matching_file(folder: str):
    info(f"在目录中查找最新周报：{folder}")
    cands = [os.path.join(folder, f) for f in os.listdir(folder) if FILENAME_REGEX.match(f)]
    if not cands:
        raise FileNotFoundError("未找到任何 DRM Report W##'YY.xlsx 文件。")
    cands.sort(key=os.path.getmtime, reverse=True)
    ok(f"找到最新文件：{os.path.basename(cands[0])}")
    return cands[0]
...
def parse_week_from_filename(path: str):
    ...
    week = int(m.group(1))
    yy = int(m.group(2))
    info(f"解析文件名周次/年份：W{week}'{yy:02d}")
    return week, 2000 + yy

def next_week_token_from_filename(latest_path: str) -> str:
    info("计算下一周周次标记（基于 ISO 周）……")
...
def get_details_sheet(wb):
    ...
    info("定位 Details 工作表（若最后一张非 Details 则遍历查找）……")
    ...
    for s in wb.Sheets:
        if str(getattr(s, "Name", "")).strip().lower() == "details":
            ok("已找到名为 Details 的工作表。")
            return s
    warn("未找到名为 Details 的工作表，使用最后一张。")
    return last
        ...
        try:
            ...
            while True:
                ...
                if time.time() - start > timeout_sec:
                    raise TimeoutError(f"刷新连接超时（>{timeout_sec}s）：{nm}")
                try:
...
def set_column_formula_robust(ws, col_letter: str, start_row: int, end_row: int):
    ...
    raise RuntimeError(f"无法将公式写入 {col_letter} 列")
...
def refresh_and_update(file_path):
    ...
    try:
        ...
        try:
            ...
            info(f"读取 {READ_AD_CELL}：{ad_value}")
        except Exception as e:
            warn(f"读取 {READ_AD_CELL} 失败：{e}")

        # 末行探测
        last_row = ws.Cells(ws.Rows.Count, "G").End(-4162).Row
        if last_row < 2:
            last_row = 2
        info(f"数据区末行：{last_row}")
...
def main():
    ...
    if not os.path.exists(target_path):
        ...
        info(f"目标文件已存在，直接使用：{target_path}")
